 ...
 *
 * Generates rss files for calendar entries.
 * 1. For all entries of specified period, /rss/calendar/period/date_from-date_to
 * 	for example rss/calendar/period/20070501-20070528
 * 2. For all entries of specified group, /rss/calendar/group/groups_id
 * 2.1. For all entries of specified group and specified period, /rss/calendar/group/groups_id/date_from-date_to
 * 3. For all entries of specified ledger, /rss/calendar/ledger/ledger_id
 * 3.1. For all entries of specified ledger and specified period, /rss/calendar/ledger/ledger_id/date_from-date_to
 * 4. For all entries of specified events (or just one event), /rss/calendar/events/type1+type2+type3
 * 		or just /rss/calendar/events/type1 for single event
 * 4.1 For all entries of specified events and specified period, /rss/calendar/events/type1+type2+type3/20070219-20070319
 * 		or just /rss/calendar/events/type1/20070219-20070319 for single event
 */

public class CalendarRSSProducer extends RSSAbstractProducer implements RSSProducer {

	private List<String> rssFileURIsCacheList;

	private List<String> rssFileURIsCacheListByPeriod = new ArrayList<String>();
	private List<String> rssFileURIsCacheListByGroup = new ArrayList<String>();
	private List<String> rssFileURIsCacheListByLedger = new ArrayList<String>();
	private List<String> rssFileURIsCacheListByEvents = new ArrayList<String>();
...
//		String feedFile = "ledger_"+extraURI.substring("ledger/".length(), extraURI.length()-1)+"_"+iwc.getLocale().getLanguage()+".xml";
	...
	private String getFeed(String title, String feedFileName, Collection entries, RSSRequest rssRequest, IWContext iwc){
		...
		try {
			...
			getRepository().uploadFileAndCreateFoldersFromStringAsRoot(PATH_TO_FEED_PARENT_FOLDER, feedFileName, feedContent, RSSAbstractProducer.RSS_CONTENT_TYPE);
	...
	public void clearPeriodList(String entryBeginDate, String entryEndDate){
		...
		for (int i = 0; i < rssFileURIsCacheListByPeriod.size(); i++) {
			entry = rssFileURIsCacheListByPeriod.get(i);
