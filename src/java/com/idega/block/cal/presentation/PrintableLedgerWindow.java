...
/**
 * Description: A printable version of the LedgerWindow - displays the same
 * information <br>
 * except without any buttons and unnecessary elements Copyright: Idega Software
 * 2004 <br>
...
public class PrintableLedgerWindow extends Window {
	...
	private void initializeTexts(IWContext iwc) {
		...
		this.creatorText = new Text(iwrb.getLocalizedString(
				LedgerWindow.creatorFieldParameterName, "User"));
		this.creatorText.setStyleClass(bold);
		this.otherCoachesText = new Text(iwrb.getLocalizedString(
				LedgerWindow.otherCoachesFieldParameterName, "Other coaches"));
		this.otherCoachesText.setStyleClass(bold);
		this.clubText = new Text(iwrb.getLocalizedString(
				LedgerWindow.clubFieldParameterName, "Club"));
		this.clubText.setStyleClass(bold);
		this.divisionText = new Text(iwrb.getLocalizedString(
				LedgerWindow.divisionFieldParameterName, "Division"));
		this.divisionText.setStyleClass(bold);
		this.groupText = new Text(iwrb.getLocalizedString(
				LedgerWindow.groupFieldParameterName, "Group"));
		this.groupText.setStyleClass(bold);
		this.dateText = new Text(iwrb.getLocalizedString(
				LedgerWindow.dateFieldParameterName, "Date"));
	...
	private void initializeFields(IWContext iwc) {
		...
		LedgerVariationsHandler ledgerVariationsHandler = LedgerWindow
				.getLedgerVariationsHandler(iwc);

		String lIDString = iwc.getParameter(LedgerWindow.LEDGER);
		Integer lID = new Integer(lIDString);
		CalendarLedger ledger = getCalendarBusiness(iwc).getLedger(
				lID.intValue());
		...
		int groupID = getCalendarBusiness(iwc).getLedger(lID.intValue())
				.getGroupID();
		try {
			this.groupNameField = getGroupBusiness(iwc).getGroupByGroupID(
					groupID).getName();
			Collection c = getGroupBusiness(iwc).getParentGroupsRecursive(
					getGroupBusiness(iwc).getGroupByGroupID(groupID));
			if (c != null) {
		...
		if (parentGroups != null) {
			this.divisionNameField = ledgerVariationsHandler
					.getParentGroupName(parentGroups);
			this.clubNameField = ledgerVariationsHandler
					.getParentOfParentGroupName(parentGroups);
		}
		if (this.divisionNameField == null) {
			this.divisionNameField = iwrb.getLocalizedString(
					"ledgerwindow.no_division_text", "No division");
		}
		if (this.clubNameField == null) {
			this.clubNameField = iwrb.getLocalizedString(
					"ledgerwindow.no_club_text", "No club");
		}

		Timestamp fromD = getCalendarBusiness(iwc).getLedger(lID.intValue())
				.getDate();
		IWTimestamp iwFromD = new IWTimestamp(fromD);
		// fromDate is the start date of the ledger
		this.fromDateField = iwFromD.getDateString("dd. MMMMMMMM yyyy",
		...
		if (marks != null) {
			...
			while (markIter.hasNext()) {
				...
				this.allowedMarksField.add(": " + mark.getMarkDescription(), 4,
						1);
	...
	public Table getAttendanceList(IWContext iwc) {
		...
		if (ledID != -1) {
			try {
				...
				entryList = (List) calBiz.getPracticesByLedIDandMonth(ledID, m,
						y);
			} catch (Exception ex) {
		...
		if (userList != null) {
			Collections.sort(userList, new Comparator() {
				public int compare(Object arg0, Object arg1) {
					return collator.compare(((User) arg0).getName(),
							((User) arg1).getName());
				}
			});
		}
		Collections.sort(entryList, new Comparator() {
			public int compare(Object arg0, Object arg1) {
				return ((CalendarEntry) arg0).getDate().compareTo(
						((CalendarEntry) arg1).getDate());
			}
		});
		...
		while (userIter.hasNext()) {
			...
			Phone phone = null;
			try {
				phone = getUserBusiness(iwc).getUsersMobilePhone(user);
			...
			if (phone != null) {
				nameField.append(" (");
				nameField.append(phone.getNumber());
				nameField.append(")");
			} else {
				try {
				...
				if (phone != null) {
					...
					try {
						Phone phones[] = getUserBusiness(iwc).getUserPhones(
								user);
						if (phone != null && phones.length > 0) {
							phone = phones[0];
							nameField.append(" (");
							nameField.append(phone.getNumber());
							nameField.append(")");
			...
			nameTable.add(nameField.toString(), 1, 1);
			...
			while (entryIter.hasNext()) {
				...
				AttendanceEntity attendance = calBiz
						.getAttendanceByUserIDandEntry(userID.intValue(), entry);
				if (attendance != null
						&& ledgerData.equals(LedgerWindow.PRINT_WITH_DATA)) {
	...
	public CalBusiness getCalendarBusiness(IWApplicationContext iwc) {
		...
		if (calBiz == null) {
			try {
				calBiz = (CalBusiness) com.idega.business.IBOLookup
						.getServiceInstance(iwc, CalBusiness.class);
	...
	protected UserBusiness getUserBusiness(IWApplicationContext iwc) {
		...
		if (userBusiness == null) {
			try {
				userBusiness = (UserBusiness) com.idega.business.IBOLookup
