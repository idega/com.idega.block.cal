...
public abstract class AbstractScheduleRenderer extends Renderer implements Serializable {
	...
	public void encodeBegin(FacesContext context, UIComponent component) throws IOException {
		...
		writer.writeAttribute(HTML.NAME_ATTR, schedule.getClientId(context), "clientId");
		writer.endElement(HTML.INPUT_ELEM);
		// hidden input field containing the id of the last clicked date
		writer.startElement(HTML.INPUT_ELEM, schedule);
		writer.writeAttribute(HTML.TYPE_ATTR, "hidden", null);
		writer.writeAttribute(HTML.NAME_ATTR, schedule.getClientId(context) + "_last_clicked_date", "clicked_date");
		writer.endElement(HTML.INPUT_ELEM);
		...
		writer.startElement(HTML.INPUT_ELEM, schedule);
		writer.writeAttribute(HTML.TYPE_ATTR, "hidden", null);
		writer.writeAttribute(HTML.NAME_ATTR, schedule.getClientId(context) + "_last_clicked_y", "clicked_y");
		writer.endElement(HTML.INPUT_ELEM);
	}

	/**
	 * <p>
	 * Get the String representation of a date, taking into account the specified
	 * date format or the current Locale.
	 * </p>
	 * 
	 * @param context
	 *          the FacesContext
		...
		if (iwc == null) {
			...
			if ((pattern != null) && (pattern.length() > 0)) {
				...
				if (context.getApplication().getDefaultLocale() != null) {
					format = DateFormat.getDateInstance(DateFormat.MEDIUM, context.getApplication().getDefaultLocale());
				}
				else {
	...
	protected String getHeaderDateFormat(UIComponent component) {
		// first check if the headerDateFormat property is a value binding
		// expression
	...
	/**
	 * The user of the Schedule component can customize the look and feel by
	 * specifying a custom implementation of the ScheduleEntryRenderer. This
	 * method gets an instance of the specified class, or if none was specified,
	 * takes the default.
	 * 
	 * @param component
	 ...
	 *          the schedule component
	 * @param dateId
	 *          the string identifying the date
	 * @param yPos
